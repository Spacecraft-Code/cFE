...
#include \"%s_mission_cfg.h\"""" % (tmpStr, tmpStr, lcApp, lcApp)
...
typedef struct
{
    ...
    /* Input data - from I/O devices or subscribed from other apps' output data.
       Data structure should be defined in %s/fsw/mission_inc/%s_private_types.h */
    %s_InData_t   InData;

    /* Output data - to be published at the end of a Wakeup cycle.
       Data structure should be defined in $MISSION_%s/apps/inc/%s_%s_types.h */
    %s_OutData_t  OutData;
...
** End of file %s_app.h
**=====================================================================================*/
    """ % (lcApp, lcApp, ucApp, g_Mission, g_Mission, lcApp, ucApp, g_Mission, g_Mission, \
           lcApp, ucApp, ucApp, ucApp, ucApp, ucApp, ucApp, ucApp, ucApp, ucApp, ucApp, \
...
int32 %s_InitPipe()
{
    ...
    if (iStatus == CFE_SUCCESS)
    {
        CFE_SB_Subscribe(%s_WAKEUP_MID, g_%s_AppData.SchPipeId);
    }
    else
    {
...
void %s_ProcessNewData()
{
    ...
    while (bGotNewMsg)
    {
        if (CFE_SB_RcvMsg(&TlmMsgPtr, g_%s_AppData.TlmPipeId, CFE_SB_POLL) == CFE_SUCCESS)
        {
    ...
    """ % (ucApp, ucApp, ucApp, g_Owner, g_Date, ucApp, ucApp, ucApp, ucApp, ucApp)
...
void %s_ProcessNewCmds()
{
    ...
    while (bGotNewMsg)
    {
        if (CFE_SB_RcvMsg(&CmdMsgPtr, g_%s_AppData.CmdPipeId, CFE_SB_POLL) == CFE_SUCCESS)
        {
